...
bool DEBUG = true;
volatile unsigned long lastClientMillis = 0;

// Non-blocking STM32 command system
struct STM32Command {
  String command;
...
const unsigned long STM32_SEND_INTERVAL = 20; // 20ms between commands
...
void setRoutes() {
  httpd_uri_t routes[] = {
    {"/setSpeed", HTTP_POST, handle_setSpeed, NULL},
    {"/setServo", HTTP_POST, handle_setServo, NULL},
    {"/dumper", HTTP_POST, handle_dumper, NULL},
    {"/gripper", HTTP_POST, handle_gripper, NULL},
    {"/tof", HTTP_GET, handle_tof, NULL},
    {"/auto/pickup", HTTP_POST, handle_autoPickup, NULL},
    {"/auto/release", HTTP_POST, handle_autoRelease, NULL},
    {"/emergency", HTTP_POST, handle_emergency, NULL},
    {"/health", HTTP_GET, handle_health, NULL},
    {"/status", HTTP_GET, handle_status, NULL}
  };
  
  for (int i = 0; i < 9; i++) {
    httpd_register_uri_handler(http_server, &routes[i]);
  }
}